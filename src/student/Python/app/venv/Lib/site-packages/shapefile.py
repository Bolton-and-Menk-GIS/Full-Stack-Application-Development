...
class Shape(object):
    ...
    def __geo_interface__(self):
        if not self.parts or not self.points:
            Exception(
                'Invalid shape, cannot create GeoJSON representation. Shape type is "%s" but does not contain any parts and/or points.' %
...
class Writer(object):
    ...
    def __bbox(self, s):
        ...
        if len(s.points) > 0:
            ...
            raise Exception(
                "Cannot create bbox. Expected a valid shape with at least one point. Got a shape of type '%s' and 0 points." % s.shapeType)
        bbox = [min(x), min(y), max(x), max(y)]
        # update global
        if self._bbox:
            # compare with existing
            self._bbox = [min(bbox[0], self._bbox[0]), min(bbox[1], self._bbox[1]), max(bbox[2], self._bbox[2]),
    ...
    def __shpRecord(self, s):
        ...
        if s.shapeType != NULL and s.shapeType != self.shapeType:
            raise Exception(
                "The shape's type (%s) must match the type of the shapefile (%s)." % (s.shapeType, self.shapeType))
        ...
        if s.shapeType in (13, 15, 18, 31):
            try:
                ...
                raise ShapefileException(
                    "Failed to write elevation extremes for record %s. Expected floats." % self.shpNum)
            try:
                if hasattr(s, "z"):
                ...
                raise ShapefileException(
                    "Failed to write elevation values for record %s. Expected floats." % self.shpNum)
        ...
        if s.shapeType in (13, 15, 18, 23, 25, 28, 31):
            try:
                ...
                raise ShapefileException(
                    "Failed to write measure extremes for record %s. Expected floats" % self.shpNum)
            try:
                if hasattr(s, "m"):
                    ...
                    [f.write(pack("<d", p[mpos] if len(p) > mpos and p[mpos] is not None else NODATA)) for p in
                     s.points]
            except error:
        ...
        if s.shapeType == 11:
            ...
            if hasattr(s, "z"):
                ...
                try:
                    ...
                    raise ShapefileException(
                        "Failed to write elevation value for record %s. Expected floats." % self.shpNum)
            else:
                # if z values are stored as 3rd dimension
                try:
                    if len(s.points[0]) < 3:
        ...
        if s.shapeType in (11, 21):
            ...
            if hasattr(s, "m"):
                ...
                try:
                    ...
                    raise ShapefileException(
                        "Failed to write measure value for record %s. Expected floats." % self.shpNum)
            else:
                # if m values are stored as 3rd/4th dimension
                # 0-index position of m value is 3 if z type (x,y,z,m), or 2 if m type (x,y,m)
                try:
...
def test(**kwargs):
    ...
    with open("README.md", "rb") as fobj:
        test = doctest.DocTestParser().get_doctest(string=fobj.read().decode("utf8"), globs={}, name="README",
                                                   filename="README.md", lineno=0)
